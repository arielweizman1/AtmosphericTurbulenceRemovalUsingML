            ...
            if i % opt.sample_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    if batches_done == 0:
                        ...
                        GT_image_fixed = convert_im(GT_image_fixed, os.path.join(log_dir, "images/GT_image.png"), nrow=5, normalize=False,
                                                    save_im=True)
                        writer.add_image('images/real', GT_image_fixed, batches_done)

                    fake_img_fixed = denormalize(fake_img_fixed)
                    fake_img_fixed = convert_im(fake_img_fixed, os.path.join(log_dir, "images/%d.png" % batches_done), nrow=5, normalize=False,
                                                save_im=True)
